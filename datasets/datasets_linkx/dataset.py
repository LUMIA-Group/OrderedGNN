...
def load_arxiv_year_dataset(nclass=5):
    ...
    ogb_dataset = NodePropPredDataset(root='datasets/datasets_pyg', name='ogbn-arxiv')
...
def load_proteins_dataset():
    ...
    ogb_dataset = NodePropPredDataset(root='datasets/datasets_pyg', name='ogbn-proteins')
...
def load_ogb_dataset(name):
    ...
    ogb_dataset = NodePropPredDataset(root='datasets/datasets_pyg', name=name)
    dataset.graph = ogb_dataset.graph
    dataset.graph['edge_index'] = torch.as_tensor(dataset.graph['edge_index'])
    dataset.graph['node_feat'] = torch.as_tensor(dataset.graph['node_feat'])
